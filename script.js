...
document.addEventListener('DOMContentLoaded', () => {
    ...
    renderOpinions();          // Cargar opiniones públicas
    renderAdminArticles();     // No harán nada si no hay sesión
    renderAdminOpinions();
    renderAdminMessages();
    setupEventListeners();
    checkAdminSession();       // Verificar sesión existente
});
...
async function handleSubmitOpinion() {
    ...
    try {
        ...
        renderOpinions(); // recargar lista pública
    } catch (error) {
...
async function handleContactSubmit(e) {
    ...
    try {
        ...
        renderAdminMessages(); // actualizar panel si está abierto
    } catch (error) {
...
async function handleAdminLogin() {
    ...
    try {
        ...
        if (adminError) adminError.textContent = 'Credenciales incorrectas.';
...
async function deleteOpinion(id) {
    ...
    try {
        ...
        if (error) throw error;
        renderAdminOpinions();
        renderOpinions(); // actualizar lista pública
    } catch (error) {
        alert('Error al eliminar.');
        console.error(error);
    }
